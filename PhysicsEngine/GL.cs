...
namespace OpenGL
{
    ...
    public static unsafe partial class GL
    {
        ...
        {
            IntPtr buffer = new IntPtr(_glGetString(name));
        ...
        {
            IntPtr buffer = new IntPtr(_glGetStringi(name, index));
            return PtrToStringUtf8(buffer);
        }
...
xOffset, yOffset, zOffset, width, height, depth, format, type, pixels.ToPointer());
        ...
        public static void glMultiDrawElements(int mode, int[] count, int type, IntPtr indices, int drawCount)
        {
            ...
            void** ptr = (void**)indices.ToPointer();
            fixed (int* c = &count[0])
            {
        ...
        {
            int count;
            uint[] shaders = new uint[maxCount];
        ...
        {
            _glGetVertexAttribPointerv(index, pname, out IntPtr pointer);
            return pointer;
        }
        ...
        {
            _glGetBufferPointerv(target, pname, out IntPtr pointer);
            return pointer;
        }
        ...
        {
            byte[] buffer = Encoding.UTF8.GetBytes(uniformBlockName);
            fixed (byte* b = &buffer[0])
            {
        ...
        {
            uint index;
            byte[][] bytes = new[] { Encoding.UTF8.GetBytes(uniformName) };
        ...
        {
            IntPtr buffer = Marshal.AllocHGlobal(bufSize);
            _glGetTransformFeedbackVarying(program, index, bufSize, out int length, out size, out type, buffer);
            name = PtrToStringUtf8(buffer, length);
            Marshal.FreeHGlobal(buffer);
        }
